...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        public ActionResult Edit(int id)
        {
            ...
            //ashish start
            var dataImage = _productService.getPeoductImageAvalable(id);
            var category = _categoryService.GetProductCategoriesByProductId(model.Id);
            bool result = false;
            foreach (var item1 in category)
            {
                if (item1.CategoryId == 17)
                {
                    result = true;
                }
            }
            model.colorImage.cat = result;
            foreach (var item in dataImage)
            {
                //CustomizedImageModel obj = new CustomizedImageModel();
                ...
                //model.colorImage.ListData.Add(obj);
                ProductModel obj = new ProductModel();
                obj.colorImage.ImageId = item.ImageId;
                obj.colorImage.ImageIdividualFlag = item.ImageIdividualFlag;
                obj.colorImage.ImageLeftEar = item.ImageLeftEar;
                obj.colorImage.ImageRightEar = item.ImageRightEar;
                obj.colorImage.ProductID = item.ProductID;
                obj.colorImage.AlienearColorMaster.ColorId = item.AlienearColorMaster.ColorId;
                obj.colorImage.AlienearColorMaster.ColorImage = item.AlienearColorMaster.ColorImage;
                obj.colorImage.AlienearColorMaster.ColorName = item.AlienearColorMaster.ColorName;
                model.colorImage.ListData.Add(obj.colorImage);
        ...
        [HttpPost]
        public ActionResult EditWithImage(FormCollection fcol, List<HttpPostedFileBase> LeftImage, List<HttpPostedFileBase> RightImage)
        {
            string productId = fcol["Id"];
            //if (LeftImage[0].ContentLength > 0 && RightImage[0].ContentLength > 0
            //    || LeftImage[1].ContentLength > 0 && RightImage[1].ContentLength > 0)
            //{
            string id = fcol.GetValue("item.AlienearColorMaster.ColorId").AttemptedValue.ToString();
            string flag = fcol["item.ImageIdividualFlag"].Replace("true,false", "true");
            string[] ids = id.Split(',');
            string[] isSingle = flag.Split(',');
            string filesIds = "";
            string finlaId = "";
            int idsCheck = 0;
            int fileChange = 0;
            ...
            {
                if (_productService.CheckProductCount(Convert.ToInt32(item), Convert.ToInt32(productId)) == 0)
                {
                    //if (LeftImage[fileChange] != null && RightImage[fileChange] != null)//) isSingle[idsCheck] == "true")
                    if (isSingle[idsCheck] == "true")
                    {
                        if (LeftImage[fileChange] != null && RightImage[fileChange] != null)
                        {
                            finlaId += item + ",";
                            filesIds += fileChange + ",";
                        }
            ...
            if (!string.IsNullOrEmpty(finlaId) || !string.IsNullOrEmpty(filesIds))
            {
                finlaId = finlaId.Substring(0, finlaId.Length - 1);
                filesIds = filesIds.Substring(0, filesIds.Length - 1);
                string[] ids1 = finlaId.Split(',');
                string[] filesIds1 = filesIds.Split(',');
                int i = 0;
                foreach (var item1 in ids1)
                {
                    if (LeftImage[Convert.ToInt32(filesIds1[i])].ContentLength > 0 && RightImage[Convert.ToInt32(filesIds1[i])].ContentLength > 0)
                    {
                        Download dl = new Download();
                        dl.DownloadGuid = Guid.NewGuid();
                        AlienearImageColorMapping obj = new AlienearImageColorMapping();
                        LeftImage[Convert.ToInt32(filesIds1[i])].SaveAs(Server.MapPath("~/Content/Images/" + (dl.DownloadGuid) + ".png"));
                        obj.ImageLeftEar = "/Content/Images/" + (dl.DownloadGuid) + ".png";
                        Download dl1 = new Download();
                        dl1.DownloadGuid = Guid.NewGuid();
                        RightImage[Convert.ToInt32(filesIds1[i])].SaveAs(Server.MapPath("~/Content/Images/" + (dl1.DownloadGuid) + ".png"));
                        obj.ImageRightEar = "/Content/Images/" + (dl1.DownloadGuid) + ".png";
                        //if (flag == "true")
                        //{
                        ...
                        //}
                        obj.ColorID = Convert.ToInt16(item1);
                ...
                string[] ids2 = finlaId.Split(',');
                if (ids2.Length > 0 && !string.IsNullOrEmpty(productId) && isSingle.Length > 0)
                {
                    for (int j = 0; j < ids2.Length; j++)
                    {
                        _productService.UpdateStatus(Convert.ToInt32(ids2[j]), Convert.ToInt32(productId), true); //Convert.ToBoolean(isSingle[j]));
                    }
                }
            }


            //string test = Request.Form.GetValues("ImageIdividualFlag").Length.ToString();
            //return RedirectToAction("Edit", "Product", new { id = 47 });
            //if (continueEditing)
            //{
            //selected tab
        ...
        [ValidateInput(false)]
        public ActionResult ProductSpecificationAttributeAdd(ProductSpecificationAttributeModel model, int attributeTypeId, int specificationAttributeOptionId,
            string customValue, bool allowFiltering, bool showOnProductPage,
            int displayOrder, int productId, int attID)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            {
                var psa = _specificationAttributeService.GetProductSpecificationAttributeById(attID);

                if (psa != null)
                {
                    psa.AttributeTypeId = attributeTypeId;
                    psa.SpecificationAttributeOptionId = specificationAttributeOptionId;
                    psa.ProductId = productId;
                    psa.CustomValue = customValue;
                    psa.AllowFiltering = allowFiltering;
                    psa.ShowOnProductPage = showOnProductPage;
                    psa.DisplayOrder = displayOrder;
        ...
        public ActionResult ProductSpecAttrUpdate(ProductSpecificationAttributeModel model)
        {
            ...
            //psa.AttributeTypeId = model.AttributeTypeName;
        ...
        public ActionResult EditProductSpecAttr(int spid)
        {
            //if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
            //	return AccessDeniedView();

            var psa = _specificationAttributeService.GetProductSpecificationAttributeById(spid);
            if (psa == null)
            ...
            var psaModel = new ProductSpecificationAttributeModel
            {
                ...
                SpecificationAttributeOptionId = psa.SpecificationAttributeOption.SpecificationAttributeId,
            ...
            //	{
            //		//return Content("This is not your product");
            //		return Json("This is not your product");
            //	}
            //}
