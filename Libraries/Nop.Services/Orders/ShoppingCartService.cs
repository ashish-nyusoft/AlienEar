...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        public virtual ShoppingCartItem GetEntryID(int id)
        {
            ShoppingCartItem query = (from sci in _sciRepository.Table
                                      where sci.Id == id
                                      select sci).First();

            return query;
        }
        public virtual int GetLastEntryID(int id)
        {
            var query = (from sci in _sciRepository.Table
                         where sci.CustomerId == id
                         orderby sci.Id descending
                         select sci).First();

            return Convert.ToInt32(query.Id);
        }
        public virtual int GetLastEntryProductID(int id)
        {
            var query = (from sci in _sciRepository.Table
                         where sci.CustomerId == id
                         orderby sci.Id descending
            ...
            return Convert.ToInt32(query.ProductId);
        }
        public virtual void updateShoppingItems(ShoppingCartItem obj)
        {
            _sciRepository.Update(obj);
