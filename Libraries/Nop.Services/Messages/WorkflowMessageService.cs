...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        public virtual int SendOrderPlacedStoreOwnerNotification(Order order, int languageId)
        {
            ...
            var toEmail = emailAccount.Email;
            var toName = emailAccount.DisplayName;
            //var orderItem = _orderService.GetOrderItemById(order.Id);
            //string[] colorarry = new string[10];
            //if (order.OrderItems.First().AttributesXml.ToString().Contains("Color Name"))
            //{
            //    colorarry = Split(Split(order.OrderItems.First().AttributesXml.ToString(), "<ProductAttributeValue><Value>Color Name")[1], "</Value></ProductAttributeValue>");
            //    messageTemplate.Body.Replace("%Color Name%", "Color Name " + colorarry[0]);
            //}
            //else
            //{
            //    messageTemplate.Body.Replace("%Color Name%", "");
            //}
