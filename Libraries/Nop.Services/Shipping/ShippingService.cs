...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
            ...
            int storeId = 0)
        {
            ...
            var objnewew = shippingOptionRequests;

            if (shippingOptionRequests.Count > 0)
            {
                if (_workContext.CurrentCustomer.ShippingAddress.CountryId == 1)
                {
                    List<GetShippingOptionRequest> item1231 = new List<GetShippingOptionRequest>();
                    foreach (var itemsss in shippingOptionRequests)
                    {
                        GetShippingOptionRequest item12312 = new GetShippingOptionRequest();
                        item12312.AddressFrom = itemsss.AddressFrom;
                        item12312.CityFrom = itemsss.CityFrom;
                        item12312.CountryFrom = itemsss.CountryFrom;
                        item12312.Customer = itemsss.Customer;
                        item12312.ShippingAddress = itemsss.ShippingAddress;
                        item12312.StateProvinceFrom = itemsss.StateProvinceFrom;
                        item12312.StoreId = itemsss.StoreId;
                        item12312.WarehouseFrom = itemsss.WarehouseFrom;
                        item12312.ZipPostalCodeFrom = itemsss.ZipPostalCodeFrom;
                        foreach (var item22222 in itemsss.Items)
                        {
                            if (item22222.ShoppingCartItem.Product.ProductCategories.Any(x => x.CategoryId != 17))
                                item12312.Items.Add(item22222);
                        }
                        item1231.Add(item12312);
                    }
                    shippingOptionRequests = item1231;
                }
            }
            //s_12_july end
            ...
            foreach (var srcm in shippingRateComputationMethods)
            {
                ...
                {
                    if (shippingOptionRequest.Items.Count > 0)
                    {
